...
public class RouteGuideBackend {
    private static final Logger logger = Logger.getLogger(RouteGuideBackend.class.getName());

    private final int port;
    private final Server server;

    public RouteGuideBackend(int port) throws IOException {
        this(port, RouteGuideUtil.getDefaultFeaturesFile());
    }

    /** Create a RouteGuide server listening on {@code port} using {@code featureFile} database. */
    public RouteGuideBackend(int port, URL featureFile) throws IOException {
        this(Grpc.newServerBuilderForPort(port, InsecureServerCredentials.create()),
                port, RouteGuideUtil.parseFeatures(featureFile));
    }

    /** Create a RouteGuide server using serverBuilder as a base and features as data. */
    public RouteGuideBackend(ServerBuilder<?> serverBuilder, int port, Collection<Feature> features) {
        this.port = port;
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        RouteGuideBackend server = new RouteGuideBackend(8980);
        server.start();
